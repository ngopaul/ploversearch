...
function loadDict(dict) {
  $.getJSON(dict, function (dict) {
    ...
    $.each(dict, function (key, val) {
      if (codes_used[key]) return true;
      codes_used[key] = true;
      if (val.length < 2) return true;
      var pre = summarise(val);
      if (typeof rot[pre] !== 'object') rot[pre] = {};
      if (typeof rot[pre][val] !== 'object') rot[pre][val] = [];
...
function start() {
  ...
  $.getJSON("dictionary_order.json", function (dictionaries) {
    for (var i = 0; i < dictionaries.length; i++) {
      loadDict(dictionaries[i]);
    }
  }).fail(function (xhr, stat, error) {
...
function doSearch(dict) {
  ...
  setTimeout(function () {
    ...
    var inex = $('#inexact');
    if (0 !== total) inex.text('(' + total + ' inexact match' + (total === 1 ? '' : 'es') + ')'); else if (total > 50) inex.text('(over 50 inexact matches)'); else inex.text('(no ' + (hadExact ? 'further ' : '') + 'matches)');
  }, 0);
...
function addTr(r, code, trans) {
  ...
  $.each(split, function (idx, val) {
    let decompositions_and_hints_i = decompose_get_all(val, trans);
  ...
  $.each(split, function (idx, val) {
    ...
    $.each(best_decomposition[idx], function (inner, part) {
      let td = $('<td>');
      if (part.strokes.length <= 1) td.text(part.strokes)
        .addClass('notranslate'); else {
        td.append($('<u>')
...
var meanings = [
  ...
  {from: "FP", to: "ch"},
  {
    from: "RB",
    to: "sh"
  }, {from: "PB", to: "n"}, {from: "PL", to: "m"}, {from: "BG", to: "k"}, {from: "GS", to: "ion"}, {
    from: "TH",
    to: "th"
  }, {from: "KH", to: "ch"}, {from: "SH", to: "sh"}, {from: "AOEU", to: "eye"}, {from: "AEU", to: "aa"}, {
    from: "AOE",
    to: "ee"
  }, {from: "AOU", to: "oo"}, {from: "OEU", to: "oy"}, {from: "AU", to: "aw"}, {from: "EA", to: "ea"}, {
    from: "OU",
    to: "ow"
  }, {from: "EU", to: "i"}, {from: "OE", to: "oh"}, {from: "AO", to: "oo"}, {from: "*", to: ""}, {from: "-", to: ""}];
...
function calculate_similarity_parts(parts, trans) {
  ...
  for (let i = 0; i < Math.pow(2, single_letter_positions.length); i++) {
    ...
    pronunciation_choices.push(pronunciation_choice);
  }

  // pronounce each pronunciation_choice
  let pronunciations = [];
  Metaphone3();
  for (let i = 0; i < pronunciation_choices.length; i++) {
    let pronunciation = "";
    for (let j = 0; j < pronunciation_choices[i].length; j++) {
      SetWord(pronunciation_choices[i][j]);
      Encode();
 ...
 * @param trans the translation to match
 * @param had_asterisk whether the code contains an asterisk - this allows
 *   for some modifications to the decomposition (e.g. replacing G with C or K)  // TODO add more mods
 * @return {Array} a list of all possible decompositions of the code
 *   a decomposition is a list of {} objects, each with a 'strokes' and 'hint' property
 */
function decompose_helper(code, trans, had_asterisk) {
  ...
  for (let i = 0; i < possible_matches.length; ++i) {
    ...
    let recursiveDecompositions = decompose_helper(code.substring(end), trans, had_asterisk);
    // base case
    if (recursiveDecompositions.length === 0) {
      ret.push([{
        strokes: code.substring(0, end), hint: hint
      }]);
    }
    for (let j = 0; j < recursiveDecompositions.length; ++j) {
      let recursiveDecomposition = recursiveDecompositions[j];
      let item_to_push = [{
        strokes: code.substring(0, end), hint: hint
      }];
      ret.push(item_to_push.concat(recursiveDecomposition));
    }
  }
  return ret;
}
 ...
 */
function decompose_get_all(code, trans) {
  count_recursive_calls = 0;
  if ('' === code) return [];
  let decompositions = decompose_helper(code, trans, code.includes('*'));
  let hints = [];
